...
namespace ITManagement.Infrastructure.Service
{
    public class ClientService : IClientService
    {
        ...
        public async Task AddAsync(CreateClient createClient)
        {
            ...
            var departament = await _departamentRepository.GetAsync(createClient.Departament);

            if (departament == null)
                throw new Exception($"Departament {createClient.Departament} does not exists.");

            if (await _clientRepository.GetAsync(createClient.Email) != null)
        ...
        {
            var client = await _clientRepository.GetAsync(email);
            return client;
        }
